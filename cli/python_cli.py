...
def calculate_expression(arg=""):
    expr = arg if arg else input("계산할 수식을 입력하세요 (예: 10 * 5 / 2) >> ").strip()
    try:
        result = eval(expr)
        print(f"결과: {result}")
...
def view_logs(arg=""):
    log_file = arg if arg else "./cli/log.txt"
...
def backup_data(arg=""):
    target_dir = arg if arg else input("백업할 원본 폴더 경로를 입력하세요 (예: ./temp) >> ").strip()
...
def find_files(arg=""):
    if arg:
        if os.path.isdir(arg):
            ...
            target_dir, pattern = os.path.split(arg)
            if not target_dir:
...
def execute_ping(arg=""):
    target = arg if arg else input("ping 테스트를 수행할 IP 또는 도메인을 입력하세요 >> ").strip()
    if not target:
        print("대상을 입력해주세요.")
        return
    ...
    while True:
        try:
            ...
            if cmd in commands:
                ...
                print(f"'{cmd}'은(는) 알 수 없는 명령어입니다. 'help'를 입력하세요.")
